...
/*public class MetaTileEntityPrimitiveSmelter extends MultiblockControllerBase {
    ...
    protected IBlockState getCasingState() {
        return MetaBlocks.SSP_METAL_CASING.getState(BlockSSPMetalCasing.SSPMetalCasingType.Heater_block);
    }

    @Override
    public ICubeRenderer getBaseTexture(IMultiblockPart sourcePart) {
        return Textures.PRIMITIVE_HEATER;
    }

    @Override
    public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
        super.renderMetaTileEntity(renderState, translation, pipeline);
        Textures.PRIMITIVE_SMELTER_OVERLAY.render(renderState, translation, pipeline, getFrontFacing(), isActive());
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
            .aisle("XXX", "XXX", "XXX", "###", "###")
            .aisle("XXX", "X#X", "XXX", "#B#", "#B#")
            .aisle("XXX", "XYX", "XXX", "###", "###")
            .where('X', statePredicate(getCasingState()))
            .where('#', isAirPredicate())
            .where('Y', selfPredicate())
            .where('b',statePredicate())
            .build();
    }
